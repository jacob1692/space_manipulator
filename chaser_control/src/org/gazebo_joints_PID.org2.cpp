 ...
 *                                                                                                                 ****/
 ...
 // Update the controller
  void GazeboJointsPID::OnUpdate() 
  {
	...
	static int times=0;
	//static int time2=0; //! time of first contact
   ...
   if ( (p_b_ctrl1_>=n_persist)) { 
	...
	if (flag_IN_CONTACT_) {
		this->flag_CH_CTRL_=true; //! If the contact is detected, the hybrid control strikes in 
	}
 ...
 *                                                                                                                 ****/
  ...
  void GazeboJointsPID::JoyCallback( const sensor_msgs::Joy::ConstPtr& joy ) 
  {
    ...
    static int  buttonA2 = (int) joy->axes[bax_change_ctrl1_]; 
	if (buttonA2==-1)
			this->p_b_ctrl1_++;
