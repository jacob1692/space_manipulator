...
namespace gazebo 
{
      ...
      sdf::ElementPtr sdf) 
  {
    ...
    //!Initialization of private variables;
		...
		//!Initialization of pant states for buttons
   ...
   */
   ...
   if (p_benable_>=n_persist) { 
		...
		**/
		...
		available_thrust_=math::Vector3(force_thrusters,force_thrusters,force_thrusters).operator-(relative_force_); //! Calculate available_thrust_ for each direction;
 ...
 *                                                                                                                 ****/
      ...
      const sensor_msgs::Joy::ConstPtr& joy) 
  {
    ...
    if (flag_ENABLED_){
		...
		p_bxstop_+=joy->buttons[b_x_stop_];
		p_bystop_+=joy->buttons[b_y_stop_];
		p_bzstop_+=joy->buttons[b_z_stop_];
		p_bangleco_+=joy->buttons[b_angle_co_];
		p_borigrot_+=joy->buttons[b_orig_rot_];
	}
	last_thrust_publish_time_ = parent_->GetWorld()->GetSimTime();
  }
...
/*
   void GazeboRos3DThruster::ForceTorquePublisher(){
	   ...
	   thruster_msg_.linear=available_thrust_;
	   thruster_msg_.angular=available_torque_;
	   thrusters_pub_.publish(thuster_msg_);
