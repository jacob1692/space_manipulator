...
{ event::Events::DisconnectWorldUpdateBegin(this->update_connection_);
  ...
  ROS_INFO("Contact Sensor Disabled");
}
...
void GazeboRosBumperFlag::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  if (!this->parentSensor)
  {
   ...
   this->body_name_="target";
  
  if (_sdf->HasElement("bodyName"))
    this->body_name_ =
      _sdf->GetElement("bodyName")->Get<std::string>();
  ...
  if (_sdf->HasElement("robotNamespace")) {
    this->robot_namespace_ =
      ...
      model_name=robot_namespace_;
      this->robot_namespace_= this->robot_namespace_ + "/";
	}
  ...
  physics::WorldPtr _world = physics::get_world(this->parentSensor->WorldName().c_str());
  physics::ModelPtr parent_model_= _world->GetModel(model_name.c_str());
  
  
  if (frame_name_.c_str()!="world" &&  frame_name_.c_str()!="map")
	this->parent_link_=parent_model_->GetLink(frame_name_.c_str());
}
...
void GazeboRosBumperFlag::OnContact()
{
  ...
  this->contact_state_msg_.header.stamp.sec = (this->parent_link_->GetWorld()->GetSimTime()).sec;
  this->contact_state_msg_.header.stamp.nsec = (this->parent_link_->GetWorld()->GetSimTime()).nsec;
/*
  ...
  */
  {
    ...
    frame_pose = math::Pose(frame_pos, frame_rot);
	 ...
	 if (frame_name_.c_str()!="world" &&  frame_name_.c_str()!="map")
		frame_pose =this->parent_link_->GetWorldInertialPose();
...
if (flag_IN_CONTACT_){
  ...
  for (unsigned int i = 0; i < contactsPacketSize; ++i)
  {
    ...
    if (contact.collision1().find(body_name_)!=std::string::npos || contact.collision2().find(body_name_)!=std::string::npos ){ //! If either of the bodies is the link of interest
    ...
    for (unsigned int j = 0; j < contactGroupSize; ++j)
    {
      ...
      math::Vector3 force = frame_pose.rot.RotateVectorReverse(math::Vector3(
                              contact.wrench(j).body_1_wrench().force().x(),
                            contact.wrench(j).body_1_wrench().force().y(),
                            contact.wrench(j).body_1_wrench().force().z()));
      math::Vector3 torque = frame_pose.rot.RotateVectorReverse(math::Vector3(
      ...
      math::Vector3 normal = frame_pose.rot.RotateVectorReverse(
     ...
     //total_wrench.force.y  = - total_wrench.force.y;
     //total_wrench.force.z  = - total_wrench.force.z;
     //total_wrench.torque.x  = - total_wrench.torque.x;
     //total_wrench.torque.y  = - total_wrench.torque.y;
     //total_wrench.torque.z  = - total_wrench.torque.z; 
