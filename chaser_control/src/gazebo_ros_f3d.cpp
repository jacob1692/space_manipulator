 ...
 * 		Transform Wrench to Relative Frame
 * 
...
#include <chaser_control/gazebo_ros_f3d.h>
...
GazeboRosF3D::~GazeboRosF3D()
{
  ...
  ROS_INFO("GazeboRosF3D Disabled");
}
...
void GazeboRosF3D::Load( physics::ModelPtr _parent, sdf::ElementPtr _sdf )
{
  ...
  this->pub_=this->rosnode_->advertise<geometry_msgs::WrenchStamped>(
  std::string(this->topic_name_),1);
  
  
    //this->pub_ = this->rubosnode_->advertise<gazebo_msgs::ContactsState>(
...
{	
  ...
  math::Pose linkpose = this->link_->GetWorldInertialPose();
  ...
  // get torque on body
  //if (times==0){
	//std::cout<<this->frame_name_.c_str();
	//std::cout<<this->link_->GetName().c_str();
	//times=1;
	//}
	
	//if (this->frame_name_.c_str()== ) {
	std::size_t found = this->frame_name_.find(this->link_->GetName());
	
	if (found!=std::string::npos) {
		force = force.operator-();
		torque = torque.operator-();
		force= linkpose.rot.RotateVectorReverse(force);
		torque= linkpose.rot.RotateVectorReverse(torque);
	}

  this->lock_.lock();
  // copy data into wrench message
  this->wrench_msg_.header.frame_id = this->frame_name_;
  this->wrench_msg_.header.stamp.sec = (this->model_->GetWorld()->GetSimTime()).sec;
  this->wrench_msg_.header.stamp.nsec = (this->model_->GetWorld()->GetSimTime()).nsec;
