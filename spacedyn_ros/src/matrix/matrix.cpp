...
//! Last modification 16.02.17 by Jacob.
...
void matrix_LU( int n, double *a, double *l, double *u )
{
    ...
    matrix_cpy( n, n, (double *)a, (double *)u ); /* ｹﾔﾎ・U､ﾋ ｹﾔﾎ・A､ｳ･ﾔ｡ｼ､ｹ､・*/
    matrix_I( n, (double *)l );				  /* ｹﾔﾎ・L､ｱｰﾌｹﾔﾎﾋ､ｹ､・	 */
    
    for ( p = 0 ; p < n ; p++ ) /* ﾁﾝ､ｭｽﾐ､ｷﾁ犲釥ﾔ､､､ﾊ､ｬ､・*/
    {							/* ｹﾔﾎ・L,U､ﾋﾊｬｲｹ､・   */
	for ( i = p + 1 ; i < n ; i++ ) 
	{
	    c = u[ i*n + p ] / u[ p*n + p ];
...
//! Calculate the temporal derivative of a matrix 
void matrix_devt(int m, int n, double *A, double *A_prev, double dtemp, double *ans){
	
	double dt;
	dt=dtemp;
	if (dt<=0) {
		cerr<<"d_time is invalid, changed to 1 us"<<endl;
		dt=0.0000001;
		}
	double f = 1/dt;
	
	double *tmp1 = matrix_get(m,n);
	
	matrix_sub( m, n, A, A_prev, tmp1); //! A - A_prev
	matrix_scale(m, n, f, tmp1, ans);  //! d (A - A_prev)ij / dt
	
	matrix_cpy(m, n, A , A_prev ); //! Save the last iteration; 
