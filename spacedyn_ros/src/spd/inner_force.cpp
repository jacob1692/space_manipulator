...
//
// Function : inner_force( MODEL, Gravity, Force )
//            extract internal force of each joint with respect to each joint frame based on Inverse Dynamics
//            calculate the force on the system
//
// Output : Force[m.LINKNUM][6]
//
// s.abiko [2008.10]
//
//_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_//
...
// ----  Calculate internal force of each joint Inverse Dynamics ----
void inner_force(MODEL &m, double *Gravity, double **Force){
  ...
  */    
      ...
      matrix_cpy(6, 1, f[i], Force[i]);
      
    }
  }
  matrix_mult( 6, 6, 1, m.Xup[0], f[0], f[0]);
  
  matrix_cpy( 6, 1, f[0], Force[0]);
/*  for(i=0;i<3;i++){
    Force[i] = f[0][i];
    Force[i+3] = f[0][i+3];
  }
